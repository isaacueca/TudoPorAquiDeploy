<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /includes/tables/table_classifiedcategory.php
	# ----------------------------------------------------------------------------------------------------
...
<a href="<?=DEFAULT_URL?>/gerenciamento/classifiedcategs/category.php?category_id=<?=$category_id?>&screen=<?=$screen?>&letra=<?=$letra?><?=(($url_search_params) ? "&$url_search_params" : "")?>" class="addLINK"><?=system_showText(LANG_SITEMGR_CATEGORY_ADDNEW)?> &raquo;</a>
...
<? if ($categories) { ?>
	...
	<table border="0" cellpadding="2" cellspacing="2" class="standard-tableTOPBLUE">
		...
		<tr>
			<th><?=ucwords(system_showText(LANG_SITEMGR_CATEGORY))?> <?=system_showText(LANG_SITEMGR_TITLE)?></th>
			<? if ($path_count < 2) { ?>
		...
		foreach ($categories as $category) {
			...
			$subcategories = db_getFromDB("classifiedcategory", "category_id", $id, "all", "title");
		?>
...
<? } else { ?>
	<div class="response-msg inf ui-corner-all"><?=system_showText(LANG_SITEMGR_CLASSIFIED_CATEGORY_NORECORD)?></div>
<? } ?>

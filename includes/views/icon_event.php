<?
	...
	if ($user) {
		...
		$friend_link = "onclick='javascript:window.open(\"".EVENT_DEFAULT_URL."/emailform.php?id=".$event->getNumber("id")."&amp;receiver=friend\",\"popup\",\"toolbar=0,location=0,directories=0,status=0,width=700,height=450,screenX=0,screenY=0,menubar=0,scrollbars,resizable=0\")'";
		$include_favorites_link = "'javascript:includeInCookie(\"".$event->getNumber("id")."\",\"".EDIRECTORY_FOLDER."\",\"event\")'";
		$remove_favorites_link = "'javascript:removeFromCookie(\"".$event->getNumber("id")."\",\"".EDIRECTORY_FOLDER."\",\"event\")'";
		$print_link = "onclick='javascript:window.open(\"".EVENT_DEFAULT_URL."/print.php?id=".$event->getNumber("id")."\",\"popup\",\"\")'";
        ...
        if (SOCIAL_BOOKMARKING != "on") {
            ...
            $digg        = "href=\"http://digg.com/submit?phase=2&amp;url=".$sbmLink."&amp;title=".urlencode($event->getString("title"))."\" target=\"_blank\"";
            $delicious   = "href=\"http://del.icio.us/post?url=".$sbmLink."&amp;title=".urlencode($event->getString("title"))."\" target=\"_blank\"";
            $technorati  = "href=\"http://technorati.com/search/".$sbmLink."\" target=\"_blank\"";
            $reddit      = "href=\"http://reddit.com/submit?url=".$sbmLink."&amp;title=".urlencode($event->getString("title"))."\" target=\"_blank\"";
            $stumbleupon = "href=\"http://www.stumbleupon.com/submit?url=".$sbmLink."&amp;title=".urlencode($event->getString("title"))."\" target=\"_blank\"";
            $furl        = "href=\"http://www.furl.net/storeIt.jsp?t=".urlencode($event->getString("title"))."&amp;u=".$sbmLink."\" target=\"_blank\"";
            $fark        = "href=\"http://cgi.fark.com/cgi/fark/submit.pl?new_url=".$sbmLink."&amp;headline=".urlencode($event->getString("title"))."\" target=\"_blank\"";
            $blinklist   = "href=\"http://www.blinklist.com/index.php?Action=Blink/addblink.php&amp;Url=".$sbmLink."&amp;title=".urlencode($event->getString("title"))."\" target=\"_blank\"";
            $slashdot    = "href=\"http://slashdot.org/bookmark.pl?url=".$sbmLink."&amp;title=".urlencode($event->getString("title"))."\" target=\"_blank\"";
            $newsvine    = "href=\"http://www.newsvine.com/_wine/save?popoff=0&amp;u=".$sbmLink."&amp;h=".urlencode($event->getString("title"))."\" target=\"_blank\"";
            unset($sbmLink);
            unset($eventLevelObj);
        }
	...
	if ($level->getDetail($icon_event_level) == "y") {

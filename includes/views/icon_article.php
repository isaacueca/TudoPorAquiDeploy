<?
    ...
    if ($user) {
		...
		$friend_link = "onclick='javascript:window.open(\"".ARTICLE_DEFAULT_URL."/emailform.php?id=".$article->getNumber("id")."&amp;receiver=friend\",\"popup\",\"toolbar=0,location=0,directories=0,status=0,width=700,height=450,screenX=0,screenY=0,menubar=0,scrollbars,resizable=0\")'";
		$include_favorites_link = "'javascript:includeInCookie(\"".$article->getNumber("id")."\",\"".EDIRECTORY_FOLDER."\",\"article\")'";
		$remove_favorites_link = "'javascript:removeFromCookie(\"".$article->getNumber("id")."\", \"".EDIRECTORY_FOLDER."\",\"article\")'";
		$print_link = "onclick='javascript:window.open(\"".ARTICLE_DEFAULT_URL."/print.php?id=".$article->getNumber("id")."\",\"popup\",\"\")'";
        ...
        if (SOCIAL_BOOKMARKING != "on") {
            ...
            $digg        = "href=\"http://digg.com/submit?phase=2&amp;url=".$sbmLink."&amp;title=".urlencode($article->getString("title"))."\" target=\"_blank\"";
            $delicious   = "href=\"http://del.icio.us/post?url=".$sbmLink."&amp;title=".urlencode($article->getString("title"))."\" target=\"_blank\"";
            $technorati  = "href=\"http://technorati.com/search/".$sbmLink."\" target=\"_blank\"";
            $reddit      = "href=\"http://reddit.com/submit?url=".$sbmLink."&amp;title=".urlencode($article->getString("title"))."\" target=\"_blank\"";
            $stumbleupon = "href=\"http://www.stumbleupon.com/submit?url=".$sbmLink."&amp;title=".urlencode($article->getString("title"))."\" target=\"_blank\"";
            $furl        = "href=\"http://www.furl.net/storeIt.jsp?t=".urlencode($article->getString("title"))."&amp;u=".$sbmLink."\" target=\"_blank\"";
            $fark        = "href=\"http://cgi.fark.com/cgi/fark/submit.pl?new_url=".$sbmLink."&amp;headline=".urlencode($article->getString("title"))."\" target=\"_blank\"";
            $blinklist   = "href=\"http://www.blinklist.com/index.php?Action=Blink/addblink.php&amp;Url=".$sbmLink."&amp;title=".urlencode($article->getString("title"))."\" target=\"_blank\"";
            $slashdot    = "href=\"http://slashdot.org/bookmark.pl?url=".$sbmLink."&amp;title=".urlencode($article->getString("title"))."\" target=\"_blank\"";
            $newsvine    = "href=\"http://www.newsvine.com/_wine/save?popoff=0&amp;u=".$sbmLink."&amp;h=".urlencode($article->getString("title"))."\" target=\"_blank\"";
            unset($sbmLink);
	...
	$links .= "<li class=\"sendToFriend\"><a href=\"javascript:void(0)\" ".$friend_link.">".system_showText(LANG_ICONEMAILTOFRIEND)."</a></li><li>|</li>";
	...
	if ($level->getDetail($article->getNumber("level")) == "y") {
		$links .= "<li>|</li><li class=\"print\"><a href=\"javascript:void(0)\" ".$print_link.">".system_showText(LANG_ICONPRINT)."</a></li>";
	}

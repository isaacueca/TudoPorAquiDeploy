<?
    ...
    $icon_navbar = "";
    $icon_promotion_level = $promotion->getNumber("level");
	
    if ($user) {

		$friend_link = "onclick='javascript:window.open(\"".PROMOTION_DEFAULT_URL."/emailform.php?id=".$promotion->getNumber("id")."&amp;receiver=friend\",\"popup\",\"toolbar=0,location=0,directories=0,status=0,width=700,height=450,screenX=0,screenY=0,menubar=0,scrollbars,resizable=0\")'";
		$include_favorites_link = "'javascript:includeInCookie(\"".$promotion->getNumber("id")."\",\"".EDIRECTORY_FOLDER."\",\"promotion\")'";
		$remove_favorites_link = "'javascript:removeFromCookie(\"".$promotion->getNumber("id")."\",\"".EDIRECTORY_FOLDER."\",\"promotion\")'";
		$print_link = "onclick='javascript:window.open(\"".PROMOTION_DEFAULT_URL."/print.php?id=".$promotion->getNumber("id")."\",\"popup\",\"\")'";
        
        // SOCIAL BOOKMARKING
        if (SOCIAL_BOOKMARKING != "on") {
        } else {
        
            $promotionLevelObj = new Promotion();
            $sbmLink = PROMOTION_DEFAULT_URL."/results.php?id=".$promotion->getNumber("id");
            $digg        = "href=\"http://digg.com/submit?phase=2&amp;url=".$sbmLink."&amp;title=".urlencode($promotion->getString("title"))."\" target=\"_blank\"";
            $delicious   = "href=\"http://del.icio.us/post?url=".$sbmLink."&amp;title=".urlencode($promotion->getString("title"))."\" target=\"_blank\"";
            $technorati  = "href=\"http://technorati.com/search/".$sbmLink."\" target=\"_blank\"";
            $reddit      = "href=\"http://reddit.com/submit?url=".$sbmLink."&amp;title=".urlencode($promotion->getString("title"))."\" target=\"_blank\"";
            $stumbleupon = "href=\"http://www.stumbleupon.com/submit?url=".$sbmLink."&amp;title=".urlencode($promotion->getString("title"))."\" target=\"_blank\"";
            $furl        = "href=\"http://www.furl.net/storeIt.jsp?t=".urlencode($promotion->getString("title"))."&amp;u=".$sbmLink."\" target=\"_blank\"";
            $fark        = "href=\"http://cgi.fark.com/cgi/fark/submit.pl?new_url=".$sbmLink."&amp;headline=".urlencode($promotion->getString("title"))."\" target=\"_blank\"";
            $blinklist   = "href=\"http://www.blinklist.com/index.php?Action=Blink/addblink.php&amp;Url=".$sbmLink."&amp;title=".urlencode($promotion->getString("title"))."\" target=\"_blank\"";
            $slashdot    = "href=\"http://slashdot.org/bookmark.pl?url=".$sbmLink."&amp;title=".urlencode($promotion->getString("title"))."\" target=\"_blank\"";
            $newsvine    = "href=\"http://www.newsvine.com/_wine/save?popoff=0&amp;u=".$sbmLink."&amp;h=".urlencode($promotion->getString("title"))."\" target=\"_blank\"";
            unset($sbmLink);
            unset($promotionLevelObj);
        }
	...
	$links .= "<li><a href=\"javascript:void(0)\" ".$friend_link.">".system_showText(LANG_ICONEMAILTOFRIEND)."</a></li><li>|</li>";

	$links .= "<li class=\"addQuicklist\"><a href=".$include_favorites_link.">".system_showText(LANG_ICONQUICKLIST_ADD)."</a></li>"; 

	$links .= "<li>|</li><li><a href=\"javascript:void(0)\" onclick=\"".$promotion_link."\">".system_showText(LANG_ICONPRINT)."</a></li>";

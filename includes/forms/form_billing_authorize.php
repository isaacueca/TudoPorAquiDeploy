<?
	...
	if (AUTHORIZEPAYMENT_FEATURE == "on") {
		...
		if (!AUTHORIZE_LOGIN || !AUTHORIZE_TXNKEY) {
			echo "<p class=\"errorMessage\">".system_showText(LANG_AUTHORIZE_NO_AVAILABLE)." <a href=\"".DEFAULT_URL."/membros/help.php\" class=\"billing-contact\">".system_showText(LANG_LABEL_ADMINISTRATOR)."</a>.</p>";
			...
			if (!$stoppayment) {
				...
				<form name="authorizeform" target="_self" action="<?=DEFAULT_URL?>/membros/<?=$payment_process?>/processpayment.php?payment_method=<?=$payment_method?>" method="post">
					...
					<? if ($payment_process == "signup") { ?>
						<table width="100%" border="0" cellpadding="2" cellspacing="2">
							<tr>
								<td><p class="standardButton paymentButton"><a href="javascript:void(0);" id="authorizebutton" onclick="submitOrder();"><?=system_highlightLastWord(system_showText(LANG_LABEL_PLACE_ORDER_CONTINUE))?></a></p></td>
							</tr>
						</table>
					<? } else { ?>

...
<div id="header-form"><?=ucwords(system_showText(LANG_SITEMGR_MODIFY))?> <?=system_showText(LANG_SITEMGR_LISTING_SING)?> <?=ucwords(system_showText(LANG_SITEMGR_MENU_SETTINGS))?> - <?=$listingObj->getString("title")?></div>
...
<? if ($listingObj->needToCheckOut()) { ?>
	<div id="warning" class="informationMessage"><?=ucwords(system_showText(LANG_SITEMGR_UNPAIDITEM))?></div>
<? } ?>
			...
			<? if (MANUALPAYMENT_FEATURE == "on") { ?>
			  ...
			  <!--	<table cellpadding="0" cellspacing="0" class="table-form table-form-settings">
				...
				<div id="trans_form" class="base-table-form-account" style="display: <?=(($_POST["add_transaction"] == "1") ? "block" : "none" )?>;">
					...
					<?
					$acct_search_table_title = system_showText(LANG_SITEMGR_ACCOUNTSEARCH_SELECT)." ".system_showText(LANG_SITEMGR_LISTING_SING);

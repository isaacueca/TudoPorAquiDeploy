<?
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST")) {
		...
		if ($boolean_seckey && $validate_account && $validate_contact && $validate_article && $validate_discount) {
				...
				<html>
					...
					<body>
						<div class=\"email_style_settings\">
							...
							$sitemgr_msg .= ucwords(ARTICLE_FEATURE_NAME).":<br /><br />";
							$sitemgr_msg .= "<b>Title: </b>".$article->getString("title")."<br />";
							$sitemgr_msg .= "<br /><a href=\"".DEFAULT_URL."/gerenciamento/article/view.php?id=".$article->getNumber("id")."\" target=\"_blank\">".DEFAULT_URL."/gerenciamento/article/view.php?id=".$article->getNumber("id")."</a><br /><br />
						</div>
					</body>
				</html>";
			...
			if ($payment_method == "checkout") {
				header("Location: ".DEFAULT_URL."/membros/article/article.php?id=".$article->getNumber("id")."&process=signup");
	...
	$formloginaction = ((SSL_ENABLED == "on" && FORCE_MEMBERS_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/membros/login.php?destiny=".EDIRECTORY_FOLDER."/membros/article/article.php";
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function orderCalculate() {
			...
			if (xmlhttp) {
				...
				xmlhttp.open("GET", "<?=DEFAULT_URL;?>/ordercalculateprice.php?item=article&level="+get_level+"&discount_id="+get_discount_id, true);
				xmlhttp.send(null);
			}
		}
	...
	<div id="formNewUser" class="<? if ((!$usertype) || ($usertype == "newuser")) echo "isVisible"; else echo "isHidden"; ?>">
		...
		<? if ($message_account || $message_article || $message_discount || $message_contact) { ?>
			<table align="center" border="0" cellpadding="2" cellspacing="2" class="standardMessage">
				...
				<? if ($message_article || $message_discount) { ?>
					...
					<? if ($message_article) { ?>
						<tr>
							<td colspan="2" class="errorMessage"><?=$message_article?></td>
						</tr>
					<? } ?>
					<? if ($message_discount) { ?>
						<tr>
		...
		<form name="order_article" action="<?=$_SERVER["PHP_SELF"]?>" method="post" class="standardForm">
			...
			<table align="center" border="0" cellpadding="2" cellspacing="2" class="standardSIGNUPTable">
				...
				<? } ?>
				<tr>
					<th colspan="2" class="SIGNUPTable-title"><h2 class="standardSubTitle"><?=system_showText(LANG_BILLINGINFO)?><span>(<?=system_showText(LANG_BILLINGINFO_MSG1)." ".system_showText(LANG_BILLINGINFO_MSG2_ARTICLE)?>)</span></h2></th>
				</tr>
				<tr>

<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /membros/classified/index.php
	# ----------------------------------------------------------------------------------------------------
	...
	$pageObj = new pageBrowsing("Classified", $screen, 10, (($_GET["newest"])?("id DESC"):("level DESC, title")), "title", $letra, $where);
	$classifieds = $pageObj->retrievePage();

	$paging_url = DEFAULT_URL."/membros/classified/index.php";
...
?>
	...
	<div class="mainContentExtended">
		...
		<h1 class="standardTitle"><?=system_showText(system_highlightLastWord(LANG_MENU_MANAGECLASSIFIED));?></h1>
								...
								<tr>
									<td width="50%">
										<table align="center" border="0" cellpadding="0" cellspacing="0" class="warningBOX">
											<tr>
												<th><a href="<?=DEFAULT_URL?>/membros/billing/index.php" class="warningBOXcontent"><?=system_showText(LANG_MSG_CONTINUE_TO_PAY_CLASSIFIED)?></a></th>
		...
		<? } else { ?>
			<div class="response-msg inf ui-corner-all"><?=system_showText(LANG_NO_CLASSIFIEDS_IN_THE_SYSTEM)?></div>
		<? } ?>

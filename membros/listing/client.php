...
?>
	...
	<div id="page-wrapper">
		...
		<div id="header-form"><a href="<?=DEFAULT_URL?>/membros/listing/view.php?id=<?=$listing->getNumber("id")?>"><?=system_showText(LANG_MANAGE_LISTING);?> - <?=$listing->getString("title")?> - <?=system_showText(LANG_LISTING_GALLERY);?></div>

	
		<a class="btn ui-state-default ui-corner-all" href="<?=DEFAULT_URL?>/membros/client/client.php?item_type=listing&item_id=<?=$item->getNumber("id")?>"><?=system_showText(LANG_LABEL_ADDNEWGALLERY);?><span class="ui-icon ui-icon-circle-plus"/></span></a>
		<div class="clearfix"></div>
		
	
		<? $listingLevelClient = new ListingLevel(); ?>
		<br />
		<div class="response-msg notice ui-corner-all"><?=system_showText(LANG_MSG_LISTING_WILL_SHOW)?> <?=(($listingLevelClient->getImages($item->getNumber("level")) == -1) ? (system_showText(LANG_LABEL_UNLIMITED)) : (system_showText(LANG_MSG_THE_MAX_OF)." ".$listingLevelClient->getImages($item->getNumber("level"))))." ".(($listingLevelClient->getImages($item->getNumber("level")) == 1) ? (LANG_MSG_GALLERY_PHOTO) : (LANG_MSG_GALLERY_PHOTOS)) ?> <?=system_showText(LANG_MSG_PER_GALLERY)?> </div>

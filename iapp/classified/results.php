<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /iapp/classified/results.php
	# ----------------------------------------------------------------------------------------------------
	...
	$section = "classified";
	$backButton = false;
	$headerTitle = ucwords(CLASSIFIED_FEATURE_NAME)." Results";
	$homeButton = false;
	$contactButton = false;
	$searchButton = true;
	$searchButtonLink = DEFAULT_URL."/iapp/classified/main.php";
...
?>
	...
	if ($_GET["keyword"]) {
		...
		$classifiedCategoryObj = new ClassifiedCategory($_GET["category_id"]);
		$message_searchresults = "<p class=\"searchResults\">Search results in category <strong>".$classifiedCategoryObj->getString("title")."</strong></p>";
	}
	...
	if ($result) {
		...
		if ($item_amount > 0) {
			...
			while ($classified = mysql_fetch_assoc($result)) {
				include(EDIRECTORY_ROOT."/iapp/classified/view.php");
			}

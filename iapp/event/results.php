<?
	...
	$section = "event";
	$backButton = false;
	$headerTitle = ucwords(EVENT_FEATURE_NAME)." Results";
	$homeButton = false;
	$contactButton = false;
	$searchButton = true;
	$searchButtonLink = DEFAULT_URL."/iapp/event/main.php";
...
?>
	...
	if ($_GET["keyword"]) {
		...
		$eventCategoryObj = new EventCategory($_GET["category_id"]);
		$message_searchresults = "<p class=\"searchResults\">Search results in category <strong>".$eventCategoryObj->getString("title")."</strong></p>";
	}
	...
	if ($result) {
		...
		if ($item_amount > 0) {
			...
			while ($event = mysql_fetch_assoc($result)) {
				include(EDIRECTORY_ROOT."/iapp/event/view.php");
			}

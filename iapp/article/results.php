<?
	...
	$section = "article";
	$backButton = false;
	$headerTitle = ucwords(ARTICLE_FEATURE_NAME)." Results";
	$homeButton = false;
	$contactButton = false;
	$searchButton = true;
	$searchButtonLink = DEFAULT_URL."/iapp/article/main.php";
...
?>
	...
	if ($_GET["keyword"]) {
		...
		$articleCategoryObj = new ArticleCategory($_GET["category_id"]);
		$message_searchresults = "<p class=\"searchResults\">Search results in category <strong>".$articleCategoryObj->getString("title")."</strong></p>";
	}
	...
	if ($result) {
		...
		if ($item_amount > 0) {
			...
			while ($article = mysql_fetch_assoc($result)) {
				include(EDIRECTORY_ROOT."/iapp/article/view.php");
			}

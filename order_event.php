<?
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST")) {
		...
		if ($boolean_seckey && $validate_account && $validate_contact && $validate_event && $validate_discount) {
				...
				<html>
					...
					<body>
						<div class=\"email_style_settings\">
							...
							$sitemgr_msg .= EVENT_FEATURE_NAME.":<br /><br />";
							$sitemgr_msg .= "<b>Title: </b>".$event->getString("title")."<br />";
							$sitemgr_msg .= "<br /><a href=\"".DEFAULT_URL."/gerenciamento/event/view.php?id=".$event->getNumber("id")."\" target=\"_blank\">".DEFAULT_URL."/gerenciamento/event/view.php?id=".$event->getNumber("id")."</a><br /><br />
						</div>
					</body>
				</html>";
			...
			if ($payment_method == "checkout") {
				header("Location: ".DEFAULT_URL."/membros/event/event.php?id=".$event->getNumber("id")."&process=signup");
	...
	$formloginaction = ((SSL_ENABLED == "on" && FORCE_MEMBERS_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/membros/login.php?destiny=".EDIRECTORY_FOLDER."/membros/event/eventlevel.php";
...
?>
	...
	<script language="javascript" type="text/javascript" src="<?=DEFAULT_URL?>/scripts/calendarmdy.js"></script>
	<? } elseif (DEFAULT_DATE_FORMAT == "d/m/Y") { ?>
	<script language="javascript" type="text/javascript" src="<?=DEFAULT_URL?>/scripts/calendardmy.js"></script>
	<? } ?>
	...
	<script language="javascript" type="text/javascript">
		...
		function orderCalculate() {
			...
			if (xmlhttp) {
				...
				xmlhttp.open("GET", "<?=DEFAULT_URL;?>/ordercalculateprice.php?item=event&level="+get_level+"&discount_id="+get_discount_id, true);
				xmlhttp.send(null);
			}
		}
	...
	<div id="formNewUser" class="<? if ((!$usertype) || ($usertype == "newuser")) echo "isVisible"; else echo "isHidden"; ?>">
		...
		<? if ($message_account || $message_event || $message_discount || $message_contact) { ?>
			<table align="center" border="0" cellpadding="2" cellspacing="2" class="standardMessage">
				...
				<? if ($message_event || $message_discount) { ?>
					...
					<? if ($message_event) { ?>
						<tr>
							<td colspan="2" class="errorMessage"><?=$message_event?></td>
						</tr>
					<? } ?>
					<? if ($message_discount) { ?>
						<tr>
		...
		<form name="order_event" action="<?=$_SERVER["PHP_SELF"]?>" method="post" class="standardForm">
			...
			<table align="center" border="0" cellpadding="2" cellspacing="2" class="standardSIGNUPTable">
				...
				<tr>
					<th colspan="2" class="SIGNUPTable-title"><?=system_showText(LANG_EVENTINFO)?></th>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<table class="tableOrderEvent_date" border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td class="eventDate"><input type="text" name="start_date" value="<?=$start_date?>" /> <span>(<?=format_printDateStandard()?>)</span></td>
								<td class="alignTop"><a href="javascript:cal_start.popup('', '<?=DEFAULT_URL?>');" title="Click here to select start date"><img src="<?=DEFAULT_URL?>/images/calendar/cal.gif" alt="Calendar" title="Calendar" border="0" /></a></td>
							</tr>
						</table>
					</td>
				</tr>
				<tr>
					...
					<td>
						<table class="tableOrderEvent_date" border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td class="eventDate"><input type="text" name="end_date" value="<?=$end_date?>" /> <span>(<?=format_printDateStandard()?>)</span></td>
								<td class="alignTop"><a href="javascript:cal_end.popup('', '<?=DEFAULT_URL?>');" title="Click here to select end date"><img src="<?=DEFAULT_URL?>/images/calendar/cal.gif" alt="Calendar" title="Calendar" border="0" /></a></td>
							</tr>
						</table>
					</td>
				</tr>
	...
	<script language="javascript" type="text/javascript">
		...
		<? if (DEFAULT_DATE_FORMAT == "m/d/Y") { ?>
			var cal_start = new calendarmdy(document.forms['order_event'].elements['start_date']);
		<? } elseif (DEFAULT_DATE_FORMAT == "d/m/Y") { ?>
			var cal_start = new calendardmy(document.forms['order_event'].elements['start_date']);
		<? } ?>
		cal_start.year_scroll = true;
		cal_start.time_comp = false;

		<? if (DEFAULT_DATE_FORMAT == "m/d/Y") { ?>
			var cal_end = new calendarmdy(document.forms['order_event'].elements['end_date']);
		<? } elseif (DEFAULT_DATE_FORMAT == "d/m/Y") { ?>
			var cal_end = new calendardmy(document.forms['order_event'].elements['end_date']);
		<? } ?>

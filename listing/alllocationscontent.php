	...
	if ($countries) {
		foreach ($countries as $each_country) {
			if (MODREWRITE_FEATURE != "on") {
				echo "<h2 class=\"standardSubTitle\"><a href=\"".LISTING_DEFAULT_URL."/results.php?estado_id=".$each_country["id"]."\">".$each_country["name"]."</a></h2>";
			} else {
			...
			if ($states) {
				foreach ($states as $each_state) {
					...
					if (MODREWRITE_FEATURE != "on") {
						echo "<h3><a href=\"".LISTING_DEFAULT_URL."/results.php?estado_id=".$each_country["id"]."&amp;cidade_id=".$each_state["id"]."\">".$each_state["name"]."</a></h3>";
					} else {
					...
					if ($regions) {
						...
						foreach ($regions as $each_region) {
							if (MODREWRITE_FEATURE != "on") {
								$regionsString[] = "<a href=\"".LISTING_DEFAULT_URL."/results.php?estado_id=".$each_country["id"]."&amp;cidade_id=".$each_state["id"]."&amp;bairro_id=".$each_region["id"]."\">".$each_region["name"]."</a>";
							} else {

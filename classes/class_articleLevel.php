<?
	...
	class ArticleLevel {
		...
		function ArticleLevel($lang=EDIR_DEFAULT_LANGUAGE, $listAll = false) {
			
			$this->lang = $lang;
			
			$dbObj = db_getDBObject();
            if($listAll) {
			    $sql = "SELECT * FROM ArticleLevel ORDER BY value";
            } else {
                $sql = "SELECT * FROM ArticleLevel WHERE active = 'y' ORDER BY value";
            }
		...
		function getContent($value) {
			...
			if ($this->lang != EDIR_DEFAULT_LANGUAGE) {
				$dbObj = db_getDBObject();
				$sql   = "SELECT content FROM ArticleLevel_Lang WHERE value='$value' AND lang='$this->lang'";
				$result = $dbObj->query($sql);

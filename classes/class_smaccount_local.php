<?
	...
	class SMAccountLocal extends Handle {
		...
		function SMAccountLocal($var='') {

			if (is_numeric($var) && ($var)) {
				$db = db_getDBObject();
				$sql = "SELECT * FROM SMAccountLocal WHERE id = $var";
		...
		function Save() {
			...
			if ($this->id) {
				...
				$sql = "INSERT INTO SMAccountLocal"
					. " ("
					. " updated,"
					. " entered,"
					. " username,"
		...
		function updatePassword() {
			$dbObj = db_getDBObject();
			$sql = "UPDATE SMAccountLocal SET password = ".db_formatString(md5($this->password))." WHERE id = $this->id";
			$dbObj->query($sql);
		}

		function Delete() {
			$dbObj = db_getDBObject();
			$sql = "DELETE FROM SMAccountLocal WHERE id = $this->id";
			$dbObj->query($sql);
		}

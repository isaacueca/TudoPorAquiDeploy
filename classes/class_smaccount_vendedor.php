<?
	...
	class SMAccountVendedor extends Handle {
		...
		function SMAccountVendedor($var='') {

			if (is_numeric($var) && ($var)) {
				$db = db_getDBObject();
				$sql = "SELECT * FROM SMAccountVendedor WHERE id = $var";
		...
		function Save() {
			...
			if ($this->id) {
				$sql  = "UPDATE SMAccountVendedor SET"
					. " updated       = NOW(),"
				...
				$sql = "INSERT INTO SMAccountVendedor"
					. " ("
					. " updated,"
					. " entered,"
					. " username,"
		...
		function updatePassword() {
			$dbObj = db_getDBObject();
			$sql = "UPDATE SMAccountVendedor SET password = ".db_formatString(md5($this->password))." WHERE id = $this->id";
			$dbObj->query($sql);
		}

		function Delete() {
			$dbObj = db_getDBObject();
			$sql = "DELETE FROM SMAccountVendedor WHERE id = $this->id";
			$dbObj->query($sql);
		}

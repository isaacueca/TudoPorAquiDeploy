<?
	...
	class ListingLevel {
		...
		function ListingLevel($lang = EDIR_DEFAULT_LANGUAGE, $listAll = false) {
			
			$this->lang = $lang;

			$dbObj = db_getDBObject();
            if($listAll) {
                $sql = "SELECT * FROM ListingLevel ORDER BY value";
            } else {
			    $sql = "SELECT * FROM ListingLevel WHERE active = 'y' ORDER BY value";
            }
			...
			while ($row = mysql_fetch_assoc($result)) {
				...
				$this->has_promotion[] = $row["has_promotion"];
				$this->price[] = $row["price"];
				$this->free_category[] = $row["free_category"];
				$this->category_price[] = $row["category_price"];
		...
		function getContent($value) {
			...
			if ($this->lang != EDIR_DEFAULT_LANGUAGE) {
				$dbObj = db_getDBObject();
				$sql   = "SELECT content FROM ListingLevel_Lang WHERE value='$value' AND lang='$this->lang'";
				$result = $dbObj->query($sql);

		...
		function Delete() {
			...
			if ($this->id != 0) {
				...
				while ($row = mysql_fetch_array($r)) {
					$sql = "UPDATE Classified SET cat_1_id = 0, parcat_1_level1_id = 0 WHERE cat_1_id = $row[id]";
					$dbObj->query($sql);
				}
				...
				$sql = "DELETE FROM ClassifiedCategory WHERE category_id = $this->id";
				$dbObj->query($sql);

				$sql = "UPDATE Classified SET cat_1_id = 0, parcat_1_level1_id = 0 WHERE cat_1_id = $this->id";
				$dbObj->query($sql);

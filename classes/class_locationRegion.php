<?
	...
	class LocationRegion extends Handle {
		...
		function Save() {
			...
			if ($this->id) {
				...
				$sql = "INSERT INTO Location_Region"
					. " (state_id, name, abbreviation, friendly_url, seo_description, seo_keywords)"
					. " VALUES"
		...
		function Delete() {
			$dbObj = db_getDBObJect();
			$sql = "SELECT * FROM Location_City WHERE region_id = $this->id";
		...
		function isRepeated(){
			...
			$dbObj = db_getDBObJect();
			$sql = "SELECT * FROM Location_Region WHERE name = ".db_formatString($this->name)." AND state_id = $this->cidade_id";
		...
		function retrievedIfRepeated() {
			...
			$sql = "SELECT * FROM Location_Region WHERE friendly_url = ".db_formatString($this->friendly_url)." AND state_id = $this->cidade_id";
			if ($this->id) $sql .= " AND id != $this->id";
		...
		function retrieveRegionsByState(){
			...
			$dbObj = db_getDBObJect();
			$sql = "SELECT * FROM Location_Region WHERE state_id = $this->cidade_id ORDER BY name";
			$result = $dbObj->query($sql);
			while($row = mysql_fetch_assoc($result)) $data[] = $row;
			if($data) return $data; else return false;
		}
		...
		function isValidFriendlyUrl(&$error_message) {
			...
			if($this->getString("cidade_id"))
				$sql .= " AND state_id = $this->cidade_id ";

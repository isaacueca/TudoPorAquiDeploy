<?
	...
	class EventLevel {
		...
		function EventLevel($lang=EDIR_DEFAULT_LANGUAGE, $listAll = false) {
			
			$this->lang = $lang;
			
			$dbObj = db_getDBObject();
		    if($listAll) {
        	    $sql = "SELECT * FROM EventLevel ORDER BY value";
            } else {
                $sql = "SELECT * FROM EventLevel WHERE active = 'y' ORDER BY value";
            }
		...
		function getContent($value) {
			...
			if ($this->lang != EDIR_DEFAULT_LANGUAGE) {
				$dbObj = db_getDBObject();
				$sql   = "SELECT content FROM EventLevel_Lang WHERE value='$value' AND lang='$this->lang'";
				$result = $dbObj->query($sql);

		...
		if (!$classifieds) {
			if ($search_lock) {
				...
				if ($db->getRowCount("Classified") > 0) {
					...
					?><div class="response-msg inf ui-corner-all"><?=system_showText(LANG_MSG_NOCLASSIFIEDS);?></p><?
				}
			}
		} elseif ($classifieds) {
			$itemRSSSection = "classified";
			echo "<h3 class=\"standardTitle\">".system_showText(system_highlightLastWord(LANG_MSG_CLASSIFIEDRESULTS));

<?
	...
	if ($keyword) {
		...
		if (CLASSIFIEDCATEGORY_SCALABILITY_OPTIMIZATION == "on") {
			$sql = "SELECT * FROM ClassifiedCategory WHERE category_id = 0 AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND ".$where_clause."";
		} else {
			$sql = "SELECT * FROM ClassifiedCategory WHERE lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND ".$where_clause."";
		}

<?
	...
	check_action_permission('admin_comercial', 'add');
	...
	/*	$items =  $_POST['selected_items'];
		...
		if (validate_smaccount($_POST, $message_smaccount)) {
			...
			$_POST["admin_type"] = 'admin_comercial';
			$permission_actions = array(array('usuarios', '1'), array('admin_geral', '0'), array('admin_comercial', '0'),array('admin_local', '1'),array('vendedores', '0'),array('estabelecimentos', '15'), array('investidores', '0'), array('categorias', '	1'),array('banners', '15'), array('estados', '1'),array('cidades', '1'),array('bairros', '1'),array('pagamento_faturas', '31'),array('relatorio_de_estatistica','1'));
			...
			header("Location: ".DEFAULT_URL."/gerenciamento/smaccount_comercial/".(($search_page) ? "search.php" : "index.php")."?message=".urlencode($message)."&newest=".$newest."&screen=$screen&letra=$letra".(($url_search_params) ? "&$url_search_params" : "")."");
			exit;
	...
	?>
		...
		//		fillLocation($id);
			...
			<? include(INCLUDES_DIR."/tables/table_smaccount_comercial_submenu.php"); ?>
			<?
			if($id || $account_id) 
				$prefix = "Editar";
			else 
			...
			<form name="smaccount" action="<?=$_SERVER["PHP_SELF"]?>" method="post" style="margin:0; padding:0">
				...
				<? include(INCLUDES_DIR."/forms/form_smaccount_comercial.php"); ?>

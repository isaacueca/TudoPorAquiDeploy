<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /gerenciamento/gallery/search.php
	# ----------------------------------------------------------------------------------------------------
	...
	if ($search_title) $sql_where[] = " Gallery.title LIKE '%".addslashes($search_title)."%' ";

	if ($search_section) {
		$table = "Gallery, Gallery_Item";
		$sql_where[] = " Gallery.id = Gallery_Item.gallery_id ";
		$sql_where[] = " Gallery_Item.item_type = ".db_formatString($search_section)." ";
	}

	if ($search_no_owner==1) $sql_where[] = " Gallery.account_id = 0 ";
	elseif ($search_account_id) $sql_where[] = " Gallery.account_id = $search_account_id ";
	...
	$paging_url = DEFAULT_URL."/gerenciamento/gallery/search.php";

	if ($search_submit) {
		$pageObj = new pageBrowsing($table, $screen, 10, "Gallery.title", "Gallery.title", $letra, $where, "Gallery.*", "Gallery");
...
<div id="page-wrapper">
	...
	<!-- 		<? include(INCLUDES_DIR."/tables/table_gallery_submenu.php"); ?> -->
			...
			<form name="gallery" method="get">

				<? if ($error_message) echo "<p class=\"errorMessage\">".$error_message."</p>"; ?>

				<? include(INCLUDES_DIR."/forms/form_searchgallery.php"); ?>

<?
	...
	check_action_permission('admin_local', 'add');
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (validate_smaccount($_POST, $message_smaccount)) {
			...
			$_POST["admin_type"] = 'admin_local';
			$permission_actions = array(array('usuarios', '15'), array('admin_geral', '0'), array('admin_comercial', '0'),array('admin_local', '0'),array('vendedores', '15'),array('estabelecimentos', '15'), array('investidores', '0'), array('categorias', '0'),array('banners', '15'), array('estados', '1'),array('cidades', '1'),array('bairros', '15'),array('pagamento_faturas', '0'),array('relatorio_de_estatistica','0'));
			...
			header("Location: ".DEFAULT_URL."/gerenciamento/smaccount_local/".(($search_page) ? "search.php" : "index.php")."?message=".urlencode($message)."&newest=".$newest."&screen=$screen&letra=$letra".(($url_search_params) ? "&$url_search_params" : "")."");
			exit;
	...
	?>
		...
		<div id="page-wrapper">
			...
			<form name="smaccount" action="<?=$_SERVER["PHP_SELF"]?>" method="post" style="margin:0; padding:0">
				...
				<? include(INCLUDES_DIR."/forms/form_smaccount_local.php"); ?>

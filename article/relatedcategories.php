<?
	...
	if ($keyword) {
		...
		if (ARTICLECATEGORY_SCALABILITY_OPTIMIZATION == "on") {
			$sql = "SELECT * FROM ArticleCategory WHERE category_id = 0 AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND ".$where_clause."";
		} else {
			$sql = "SELECT * FROM ArticleCategory WHERE lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND ".$where_clause."";
		}

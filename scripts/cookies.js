...
function includeInCookie(property_id, edir_path, type) {
	if (!isNaN(property_id)) {
		...
		$('#confirmDiv').html("<div class=\"response-msg notice ui-corner-all\">"+showText(LANG_JS_FAVORITEADD)+"</div>");
		tb_show("", "#TB_inline?width=400&height=125&inlineId=confirmDiv");
	}
}
...
function removeFromCookie(property_id, edir_path,type) {
	...
	if (isNaN(property_id)) {
		...
		tb_show("", "#TB_inline?width=300&height=125&inlineId=confirmDiv");

<?
	...
	if ($categories) {
		foreach ($categories as $category) {
			...
			if (EVENTCATEGORY_SCALABILITY_OPTIMIZATION != "on") {
				$subcategories = db_getFromDBBySQL("eventcategory", "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber($category->getNumber("id"))." AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." ORDER BY title");
				if ($subcategories) {
	...
	$categoryDD = html_selectBoxCat("category_id", $nameArray, $valueArray, "", "", "", system_showText(LANG_SEARCH_LABELCBCATEGORY), "event");

<?
	...
	if ($keyword) {
		...
		if (EVENTCATEGORY_SCALABILITY_OPTIMIZATION == "on") {
			$sql = "SELECT * FROM EventCategory WHERE category_id = 0 AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND ".$where_clause."";
		} else {
			$sql = "SELECT * FROM EventCategory WHERE lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND ".$where_clause."";
		}
